...
import math
from torch.nn import GroupNorm  # PyTorchの標準GroupNormをインポート
...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = conv_block(ch_in=1, ch_out=64, k_size=3, num_groups=1)
        self.res_block1 = ResNet_block(ch=64, k_size=3, num_groups=8)
        self.MaxPool1 = nn.MaxPool3d(3, stride=2, padding=1)

        self.conv2 = conv_block(ch_in=64, ch_out=128, k_size=3, num_groups=8)
        self.res_block2 = ResNet_block(ch=128, k_size=3, num_groups=16)
        self.MaxPool2 = nn.MaxPool3d(3, stride=2, padding=1)

        self.conv3 = conv_block(ch_in=128, ch_out=256, k_size=3, num_groups=16)
        self.res_block3 = ResNet_block(ch=256, k_size=3, num_groups=16)
        self.MaxPool3 = nn.MaxPool3d(3, stride=2, padding=1)

        self.conv4 = conv_block(ch_in=256, ch_out=512, k_size=3, num_groups=16)
        self.res_block4 = ResNet_block(ch=512, k_size=3, num_groups=16)
    ...
    def forward(self, x):
        ...
        # print(f"Encoder output x4: min={x4.min()}, max={x4.max()}")
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim):
        ...
        self.linear_up = nn.Linear(latent_dim, 512 * 150)
        self.relu = nn.ReLU()
        self.upsize4 = up_conv(ch_in=512, ch_out=256, k_size=1, scale=2)
        self.res_block4 = ResNet_block(ch=256, k_size=3, num_groups=16)
        self.upsize3 = up_conv(ch_in=256, ch_out=128, k_size=1, scale=2)
        self.res_block3 = ResNet_block(ch=128, k_size=3, num_groups=16)
        self.upsize2 = up_conv(ch_in=128, ch_out=64, k_size=1, scale=2)
        self.res_block2 = ResNet_block(ch=64, k_size=3, num_groups=16)
        self.upsize1 = up_conv(ch_in=64, ch_out=1, k_size=1, scale=2)
...
class VAE(nn.Module):
    def __init__(self, latent_dim=16384):
        super(VAE, self).__init__()
        self.device = (
            torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        )
        self.latent_dim = latent_dim
        self.z_mean = nn.Linear(512 * 150, latent_dim)
        self.z_log_sigma = nn.Linear(512 * 150, latent_dim)
        self.epsilon = torch.normal(
            size=(1, latent_dim), mean=0, std=1.0, device=self.device
        )
    ...
    def forward(self, x):
        ...
        # Reparameterization trick: sample epsilon for each forward pass
        ...
        print(f"Latent variable z: min={z.min()}, max={z.max()}")
        """

...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim):
        ...
        self.linear_up = nn.Linear(latent_dim, 64 * 9600)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        x2_ = self.linear_up(x)
        x2_ = self.relu(x2_)

        x2_ = x2_.view(-1, 64, 20, 24, 20)
        x2_ = self.upsize4(x2_)
        x2_ = self.res_block4(x2_)

        x1_ = self.upsize3(x1_)
...
class VAE(nn.Module):
    def __init__(self, latent_dim=8192):
        ...
        self.latent_dim = latent_dim
        self.z_mean = nn.Linear(64 * 9600, latent_dim)
        self.z_log_sigma = nn.Linear(64 * 9600, latent_dim)

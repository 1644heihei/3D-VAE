...
def load_mri_images(path, batch_size):
    ...
    while n < len(filenames):
        ...
        for i in range(n, n + batch_size):
            ...
            image = np.fromfile(filepath, dtype=np.int16).reshape((80, 96, 80))  # 必要に応じて reshape を調整
            #print(f"Loaded image: {filepath}, min: {image.min()}, max: {image.max()}")
        ...
        if batch_image:
            ...
            #print(f"Batch size: {batch_image.shape}")
            yield batch_image

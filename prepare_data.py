...
def process_cut(i, j, k, slidex, slidey, slidez, data, cutx, cuty, cutz):
    ...
    # 32x32x32 のデータを切り出し
    return data[
        :, :, d_slide : 80 + d_slide, h_slide : 96 + h_slide, w_slide : 80 + w_slide
    ]
...
def cut(y, DimSize, img17h, img17l, img18h, img18l):
    ...
    def parallel_cut(data, tmp_cutdata):
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(
                    process_cut, i, j, k, slidex, slidey, slidez, data, cutx, cuty, cutz
                )
                for i in range(cutz)
                for j in range(cuty)
                for k in range(cutx)
    ...
    # データ保存 (ペアで保存)
    def save_paired_data(lr_data_list, hr_data_list):
        nonlocal y
        # 797と835のデータを結合してループ
        combined_lr = np.concatenate([lr_data_list[0], lr_data_list[1]], axis=0)
        combined_hr = np.concatenate([hr_data_list[0], hr_data_list[1]], axis=0)

        for i in range(combined_lr.shape[0]):
            lr_patch = combined_lr[i, 0, :, :, :]
            hr_patch = combined_hr[i, 0, :, :, :]

            # データがすべてゼロの場合はスキップ (LRかHRどちらかがゼロならスキップ)
            if np.all(lr_patch == 0) or np.all(hr_patch == 0):
                # print(f"Skipping empty data at index {i}")
                continue

            lr_patch = np.clip(lr_patch, 0, 255)
            hr_patch = np.clip(hr_patch, 0, 255)

            filename = f"patch_{y:06d}.raw"
            lr_path = os.path.join(output_dir_lr, filename)
            hr_path = os.path.join(output_dir_hr, filename)

            lr_patch.astype(np.int16).tofile(lr_path)
            hr_patch.astype(np.int16).tofile(hr_path)
            y += 1

    save_paired_data(
        [tmp_LR_cutdata797, tmp_LR_cutdata835], [tmp_HR_cutdata797, tmp_HR_cutdata835]
    )

    print(f"保存完了: {y} patches processed so far.")
    return y
